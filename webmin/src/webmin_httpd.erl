...
httpd_start_link() ->
    ...
    %%inets:start(httpd, ConfigList, stand_alone).
    {ok,Pid} = httpd:start_link("./priv/webmin/conf/httpd.conf").

config()->
    ServerRoot = "./priv/webmin",
    ConfigPath = ServerRoot ++ "/conf/httpd.conf",
    io:fwrite("webmin_httpd ~p:config/0:ConfigPath=~p~n",[?LINE, ConfigPath]),
    {ok, ConfigFromFile} = httpd_conf:load(ConfigPath),
    io:fwrite("webmin_httpd ~p:config/0:ConfigFromFile=~p~n",[?LINE, ConfigFromFile]),
    ConfigOMA = [ {document_root, ServerRoot++"/htdocs"},
                  {alias, {"/icons/", ServerRoot++"/icons/"}},
                  {alias, {"/pics/", ServerRoot++"/icons/"}},
                  {server_root, ServerRoot}
                 ],
    ConfigList = ConfigOMA ++ ConfigFromFile,
